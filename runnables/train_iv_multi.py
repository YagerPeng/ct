...
def main(args: DictConfig):
    ...
    multimodel = instantiate(args.model.iv_multi, args, dataset_collection, _recursive_=False)
    if args.model.iv_multi.tune_hparams:
        multimodel.finetune(resources_per_trial=args.model.iv_multi.resources_per_trial)
    ...
    multimodel_trainer = Trainer(gpus=eval(str(args.exp.gpus)), logger=mlf_logger, max_epochs=args.exp.max_epochs,
                                 ...
                                 gradient_clip_val=args.model.iv_multi.max_grad_norm if args.model.iv_multi.max_grad_norm is not None else 0.0)
    logger.debug(f'before fit-------------------------------------------------------')
    multimodel_trainer.fit(multimodel)

    logger.debug(f'after fit-------------------------------------------------------')
